...
module.exports.checkout = async (userId, reqBody) => {
  const orderProductsIds = Promise.all(
    reqBody.orderProducts.map(async (orderProduct) => {
      // ADD CODE TO CONNECT WHICH ORDER THE ORDER-PRODUCT BELONGS TO (BY USING ORDER ID) - not achieved
      productController.subtractProductStocks(
        orderProduct.productId,
        orderProduct.quantity
      ); // Automatic subtraction to product's stocks based on the quantity on the order
...
module.exports.approveOrders = (orderId) => {
  ...
  return Order.findByIdAndUpdate(orderId, updateStatusField).then(
    (order, error) => {
      if (error) {
...
module.exports.deleteOrders = () => {
  return Order.deleteMany().then((result) => {
    return result;
  });
};
...
module.exports.deleteOrderProducts = () => {
  return OrderProduct.deleteMany().then((result) => {
    return result;
  });
};
