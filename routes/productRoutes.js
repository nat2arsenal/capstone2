...
router.post('/add', auth.verify, (req, res) => {
  if (auth.decode(req.headers.authorization).isAdmin === false) {
    res.send({
      message: 'Failed to add new product, please seek admin assistance.',
    });
  } else {
    ...
    Product.findOne({
      ...
      if (result !== null) {
        // Check if there's already a product that has the same name and description
        /* 
        ...
        res.send({
          message:
            "Product is already existing, please update the product's stock instead.",
        });
      } else {
        productController
          .addNewProduct(reqBody)
...
router.patch('/:productId/info/stocks', auth.verify, (req, res) => {
  if (auth.decode(req.headers.authorization).isAdmin === false) {
    res.send({
      message:
        "Failed to update the product's information, please seek admin assistance.",
    });
  } else {
    productController
      .addProductStocks(req.params.productId, req.body.stocks)
      .then((resultFromController) => res.send(resultFromController));
  }
});
...
router.patch('/:productId/info/stocks/subtract', (req, res) => {
  ...
  // } else {
  productController
    .subtractProductStocks(req.params.productId, req.body.stocks)
    .then((resultFromController) => res.send(resultFromController));
  // }
});
...
router.patch('/:productId/archive', auth.verify, (req, res) => {
  if (auth.decode(req.headers.authorization).isAdmin === false) {
    res.send({
      message: 'Failed to archive product, please seek admin assistance.',
    });
  } else {
    productController
...
router.patch('/:productId/activate', auth.verify, (req, res) => {
  ...
  if (auth.decode(req.headers.authorization).isAdmin === false) {
    res.send({
      message:
        "Failed to update product's active status, please seek admin assistance.",
    });
  } else {
    productController
