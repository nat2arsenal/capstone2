...
router.patch('/:userId/setAsAdmin', auth.verify, (req, res) => {
  if (auth.decode(req.headers.authorization).isAdmin === false) {
    res.send({
      message: "You can't access this feature, please seek admin assistance.",
    });
  } else {
    userController
      .setAsAdmin(req.params.userId, req)
      .then((resultFromController) => res.send(resultFromController));
  }
});
...
router.get('/detailsAll', (req, res) => {
  ...
  if (user.isAdmin === false) {
    res.send({
      message:
        "Failed to retrieve users' information, please seek admin assistance.",
    });
  } else {
    userController
      .getAllUserProfiles()
...
router.post('/:userId/checkout', auth.verify, async (req, res) => {
  if (auth.decode(req.headers.authorization).isAdmin === true) {
    ...
    if (req.params.userId === userId) {
      //Check if bearer token is the same as params.id     // OK
...
const validateOrder = async (reqBody, res) => {
  ...
  for (let i = 0; i < reqBody.orderProducts.length; i++) {
    if (reqBody.orderProducts[i].quantity > 0) {
      let isExisting = await isProductExisting(
        reqBody.orderProducts[i].productId
      );
      if (isExisting === false) {
        res.send({
          ERROR: `Order error, productId ${reqBody.orderProducts[i].productId} is not found.`,
        }); // OK up to this point
        isValid = false;
        break;
      } else {
        let isActive = await isProductActive(
          reqBody.orderProducts[i].productId
        );
        if (isActive === false) {
          res.send({
            ERROR: `Order error, productId ${reqBody.orderProducts[i].productId} is currently not active.`,
          });
          isValid = false;
          break;
        } else {
          let isEnough = await isProductEnough(
            reqBody.orderProducts[i].productId,
            reqBody.orderProducts[i].quantity
          );
          ...
          if (isEnough === false) {
            res.send({
              ERROR: `Order error, not enough stocks for productId ${reqBody.orderProducts[i].productId}. Please order a quantity of ${productStocks} or less. Thank you.`,
            });
            isValid = false;
            break;
          } else {
      ...
      res.send({
        ERROR: `Order for productId ${reqBody.orderProducts[i].productId} should contain at least 1 quantity.`,
      });
      isValid = false;
      break;
    }
